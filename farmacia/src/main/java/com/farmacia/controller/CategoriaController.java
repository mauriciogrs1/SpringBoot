...
public class CategoriaController {
	...
	@GetMapping("/{id}")
	public ResponseEntity<CategoriaModel> ListarCategoriaPorId(@PathVariable long id) {
		return repository.findById(id) .map(resp -> ResponseEntity.ok(resp)). orElse (ResponseEntity.notFound() .build());
	}
		
	@PostMapping
	public ResponseEntity<CategoriaModel> incluirCategoria(@RequestBody CategoriaModel categoria) {
